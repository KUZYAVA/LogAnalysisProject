...
public class ScheduleController {
     ...
     */
    public ScheduleController(UserService userService, ScheduleEventService scheduleEventService) {
    ...
    public String scheduleView(Model model) {
        Group group = userService.getCurrentUser().getGroup();
        model.addAttribute("groupTitle", group.getTitle());
        if (group.getHiddenByAdmin()) {
            model.addAttribute("message", "Расписание скрыто");
        } else {
            List<ScheduleEvent> scheduleEvents = scheduleEventService.findByGroupId(group.getId());
